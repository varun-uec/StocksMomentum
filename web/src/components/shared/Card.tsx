...
/** Sentinel for a metric that was never measured. Never render one as 0. */
...
export function MetricCard({
  ...
  change?: string;
  changeLabel?: string;
  /** Colour the value as a good result. Ignored when the value is UNMEASURED. */
  good?: boolean;
  /** Colour the value as a bad result. Ignored when the value is UNMEASURED. */
  bad?: boolean;
  /** Plain-language definition of the metric, shown on hover and focus. */
  hint?: string;
  color?: string;
}) {
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700/40 bg-slate-50 dark:bg-slate-800/30 px-4 py-3 shadow-sm">
      <div
        className={`text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 ${
          hint ? 'underline decoration-dotted decoration-slate-400 underline-offset-2 cursor-help w-fit' : ''
        }`}
        title={hint}
...
export type BadgeColor = 'slate' | 'emerald' | 'rose' | 'amber' | 'blue' | 'indigo';
...
export function LoadingSpinner({ text = 'Loading…' }: { text?: string }) {
  return (
    <div role="status" aria-live="polite" className="flex flex-col items-center justify-center py-20 gap-3">
      <div
        aria-hidden="true"
        className="w-6 h-6 border-2 border-slate-200 dark:border-slate-700 border-t-indigo-500 rounded-full animate-spin"
      />
      <div className="text-slate-500 dark:text-slate-400 text-sm">{text}</div>
    </div>
  );
}
...
export function ErrorMessage({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div role="alert" aria-live="assertive" className="flex items-center justify-center py-16 px-4">
      <div className="flex items-start gap-3 max-w-xl rounded-xl border border-rose-200 dark:border-rose-800/50 bg-rose-50 dark:bg-rose-950/30 p-4">
        <svg aria-hidden="true" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-rose-500 shrink-0 mt-0.5">
          <path
            fillRule="evenodd"
            clipRule="evenodd"
        ...
        <div className="min-w-0">
          ...
          {onRetry && (
            <button
              type="button"
              onClick={onRetry}
              className={`mt-3 px-3 py-1.5 rounded-lg text-xs font-medium bg-rose-600 text-white hover:bg-rose-500 ${focusRing}`}
            >
              Retry
            </button>
          )}
...
export function EmptyState({ message }: { message: string }) {
  return (
    <div role="status" className="flex flex-col items-center justify-center py-16 px-4 text-center">
      <svg aria-hidden="true" viewBox="0 0 20 20" fill="currentColor" className="w-8 h-8 text-slate-300 dark:text-slate-600 mb-3">
