"""BSE market-data provider (Phase 5.1) — official UDiFF equity bhavcopy.
...
The column schema below was read off a live 2026-08-06 download (4,962 rows),
not assumed::
...
# The equity cash segment marks every scrip ``STK``; derivatives and other
# segments are published in separate files but the guard is kept explicit.
...
class BSEBhavcopyProvider:
    ...
    async def fetch_eod(self, for_date: date) -> list[RawBar]:
        ...
        content = await self._download(for_date)
        return parse_bse_bhavcopy(content, for_date) if content else []
    ...
    async def fetch_instrument_master(self, for_date: date | None = None) -> list[RawInstrument]:
        ...
        use case does) rather than relying on the default.
        """
        session = for_date or date.today()
        content = await self._download(session)
    ...
    async def _download(self, for_date: date) -> bytes | None:
        """Fetch the raw bhavcopy payload; ``None`` on a non-session or error."""
        url = bse_bhavcopy_url(for_date)
        try:
