...
accept or reject a count, they only produce a range. The projection zone is an
Elliott Wave analytical projection for the separate research surface only -
by architectural decision it is **never** an input to the Momentum25 score, the
ranking, screening gates, the Trend Template, Relative Strength, volume or
pattern scores, the buy-setup quality, stop-loss calculation, or any production
strategy decision.
...
def subdivide(
    ...
    carries no projection zone (only the count that owns it does). Each adjacent
    label pair ``(a, b)`` bounds a
